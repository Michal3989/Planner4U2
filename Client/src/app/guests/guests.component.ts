...
export class GuestsComponent implements OnInit {
  ...
  sendInvitations() {
    ...
    swalWithBootstrapButtons
      ...
      .then(result => {
        if (result.value) {
          this.guestsService
            .sendInvitations(this.commonService.currentEvent)
            .subscribe((data: boolean) => {
              if (data) {
                swalWithBootstrapButtons.fire("כל המיילים נשלחו בהצלחה");
              } else {
                Swal.fire({
                  type: "error",
                  title: "...אופס",
  ...
  addGuest() {
    ...
    debugger;
    this.guestsService.addGuest(this.newGuest).subscribe(data => {
      this.guestsDetailsList = data;
  ...
  saveChanges() {
    this.guestsService.saveChanges(this.guestsDetailsList).subscribe(data => {
      console.log(data);
    });
  }
}
